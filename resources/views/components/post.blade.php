{{-- booléen list pour changer l'affichage du post selon qu il soit sur la page index (list true) ou sur sa page propre (list false) --}}
...
<article class="flex flex-col lg:flex-row pb-10 md:pb-16 border-b">
    <div class="lg:w-5/12">
        {{-- si le chemin de l'image commence par http, le chemin sera juste colonne thumbnail du post car on est sur une "fake" image seedée --}}
        {{-- Si non, alors on est sur une image uploadée par quelqu'un, asset mène au dossier public et on indique le sous dossier storage + lien de l'image du post --}}
        <img class="w-full max-h-72 object-cover lg:max-h-none lg:h-full" 
        src="{{ str_starts_with($post->thumbnail, 'http') ? $post->thumbnail : asset('storage/' . 
        $post->thumbnail) }}">
    </div>
    <div class="flex flex-col items-start mt-5 space-y-5 lg:w-7/12 lg:mt-0 lg:ml-12">
        @if ($post->category)
        <!-- lien avec nom de la route pour afficher la page filtrée par catégorie-->
        ...
        {{-- isNotEmpty vérifie si la collection est vide, si pas vide alors itération dans les posts qui ont des tags --}}
        ...
        @if ($list)
        <!-- lien avec nom de la route pour afficher la page du post cliqué -->
        <!-- 2e paramètre correspond a celui de PostController.php fonction show, activé par la route -->
