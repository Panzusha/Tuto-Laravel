...
<body class="antialiased pt-10 pb-16 md:pb-32">
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <header class="flex justify-between items-center space-x-5 text-slate-900">
            ...
            {{-- Navigation, x-data pour activer alpineJS --}}
            <nav x-data="{ open: false }" x-cloak class="relative">
                <button
                    @click="open = !open"
                    @click.outside="if (open) open = false"
                    @class([
                        'w-8 h-8 flex rounded-full bg-white text-sm focus:outline-none focus:ring-2 
                ...
                >
                    @auth
                    <li><a href="{{ route('home') }}" class="block px-4 py-2 text-sm text-gray-700 
                        hover:bg-gray-100">Mon compte</a></li>
                    @if (Auth::user()->isAdmin())
                        {{-- admin.posts = nom route web.php, index nom de la fonction dans AdminController.php --}}
                    <li><a href="{{ route('admin.posts.index') }}" class="block px-4 py-2 text-sm text-gray-700 
                        hover:bg-gray-100">Gestion des posts</a></li>
                    @endif
                        {{-- $refs.logout lié au x-ref="logout" en dessous (alpineJS) --}}
                    <li><a href="{{ route('logout') }}" @click.prevent="$refs.logout.submit()" class="block px-4 py-2 text-sm text-gray-700 
                        hover:bg-gray-100">Déconnexion</a></li>
                        {{-- formulaire pour gérer la déco sans utiliser http get (protection csrf) --}}
                    <form x-ref="logout" action="{{ route('logout') }}" method="POST" class="hidden">
                        @csrf
                    </form>
                    @else
                    <li><a href="{{ route('login') }}" class="block px-4 py-2 text-sm text-gray-700 
        ...
        {{-- affichage du message inscription réussie --}}
        {{-- valeur status est connue grace au return withStatus de RegisterController.php et PostController.php --}}
        @if (session('status'))
